...
public class PositionService {
    ...
    public List<VehiclePositionVO> list(String carNumber, String vehicleMask) {
        ...
        if (StringUtils.isBlank(carNumber) && StringUtils.isBlank(vehicleMask)) {
        ...
        List<VehiclePositionStatusDO> positions = getList();
        Map<Long, VehiclePositionStatusDO> vehiclePositionMap = positions.stream().collect(Collectors.toMap(VehiclePositionStatusDO::getVehicleId, v -> v));
        ...
        for (VehicleRespDTO vehicle : list) {
            ...
            VehiclePositionStatusDO position = vehiclePositionMap.get(vehicle.getId());
            if (Objects.nonNull(position)) {
                org.springframework.beans.BeanUtils.copyProperties(position, vp);
            }
